...
async function generateUniqueAppId() {
    ...
    while (!isUnique && attempts < 10) {
        appId = generateAppId();
        const existing = await (0, db_1.query)('SELECT id FROM users WHERE app_id = $1', [appId]);
...
async function register(req, res) {
    try {
       ...
       RETURNING id, username, app_id, age, gender, total_stars, total_ratings, created_at`, [userId, username, passwordHash, appId, parsedAge, gender]);
...
async function login(req, res) {
    try {
        ...
        const userResult = await (0, db_1.query)('SELECT * FROM users WHERE username = $1', [username]);
